namespace pu_ads_inf_autumn_22_common
{
    internal class Program
    {
        ...
        //Numerical systems
        //10 to binary/ binary... to very simple example
        private static string ToBinaryString(int numberInTenBasis)
        {
            String result = "";
            while (numberInTenBasis > 0)
            {
                int reminder = numberInTenBasis % 2;
                result = reminder + result;// conactinate in front
                numberInTenBasis /= 2;
            }
            return result;
        }

        private static long ToDecimalNumberFromBinaryString(string binaryString)
        {
            int power = binaryString.Length - 1;
            long result = 0;
            for (int i = 0; i < binaryString.Length; i++)
            {
                string digitStr = binaryString.Substring(i, 1);
                long digit = long.Parse(digitStr);
                if (digit != 0)
                {
        ...
        //Reminer for how the next example do the work
        private static void PrintAsciis()
        {
            int a = (int)'a';
            Console.WriteLine("a in ascii = " + a);
            int z = (int)'z';
            Console.WriteLine("z in ascii = " + z);

            int capitalA = (int)'A';
            Console.WriteLine("A in ascii = " + capitalA);
            int capitalZ = (int)'Z';
            Console.WriteLine("Z in ascii = " + capitalZ);

            int zero = (int)'0';
            Console.WriteLine("0 in ascii = " + zero);
            int nine = (int)'9';
            Console.WriteLine("9 in ascii = " + nine);
        }
